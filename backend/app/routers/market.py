...
def get_hero(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return HeroResponse(
            score=row[0] or 50, status=row[1] or "중립", emoji=row[2] or "😐",
            status_text=row[3] or "시장 분석 중", description=row[4] or "",
            action_guide=row[5] or "", data_date=str(row[6]) if row[6] else "",
        )
    except Exception:
        return default
