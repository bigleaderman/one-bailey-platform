...
async function loadMarket(dateStr) {
    try {
        const res = await fetch(`/api/predictions/date/${dateStr}/market`);
...
async function loadRecent() {
    try {
        const res = await fetch('/api/predictions/recent?days=7');
        if (!res.ok) return;
        const data = await res.json();
        renderRecent(data);
    } catch (e) {
...
function renderRecent(items) {
    ...
    grid.innerHTML = sorted.map(item => {
        ...
        const clickable = item.date_iso ? `onclick="window.location.href='/detail.html?date=${item.date_iso}'"` : '';
        return `
